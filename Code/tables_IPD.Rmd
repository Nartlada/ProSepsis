...
create_t1t_if <- function(df, overall = FALSE) {
...
create_t2t <- function(df) {
  df %>%
    zap_labels() %>%
    select(OxyInbu, vasopressor, oxygensupp, Hypoglycemia) %>%
    tbl_summary(
      label = list(
        OxyInbu      ~ 'Ventilator use',
        vasopressor  ~ 'Vasopressor use',
...
create_t2t_risk <- function(df) {
  vars <- names(sort_var(df, UnSmoke:UnAlcohol))
  df %>%
    select( vars, HIV, Covid21) %>%
    tbl_summary(
      label = list(UnSmoke   ~ 'Smoking',
                   UnAlcohol ~ 'Alcohol'),
      missing = "no",
...
create_t2t_or <- function(df) {
  df %>%
    mutate(across(SOFA2:SOFA6, ~ ifelse(. > 1, 1, .)),
           SOFA1 = ifelse(SOFA1 == 3, 1, 0)) %>%
    select( SOFA1:SOFA6, Sofa_Sepsis) %>%
    tbl_summary(
        label = list(
        SOFA1       ~ 'RS (PaO2/Fio2 >= 100 and < 200 with respiratory support',
        SOFA2       ~ 'Liver (Bilirubin > 1.2 mg/dl)',
...
create_t2t_cul_s <- function(df) {
  df %>%
    zap_labels() %>%
    select( CulBlood_ever:CulOther_ever) %>%
    tbl_summary(
      label = list(
        CulBlood_ever  ~ 'Blood',
        CulSputum_ever ~ 'Sputum',
...
create_t2t_trt <- function(df) {
  vars <- names(sort_var(df, penicillin_ever:parasitedrug_ever))
  df %>%
    zap_labels() %>%
    select( vars, otheranti_ever) %>%
    tbl_summary(
      label = list(
        penicillin_ever   ~ 'Penicillin',
        cephalG1_ever     ~ 'Cephalosporin 1st',
...
t1t_if <- create_t1t_if(df_ipd, TRUE)
...
t2t_risk <- create_t2t_risk(df_ipd)
...
t2t_cul_s <- create_t2t_cul_s(df_ipd)
...
```{r t4}
t4t_dx <- create_t4t_dx(df_ipd)
t4n_dx <- create_t4_dx(df_ipd_n)
t4s_dx <- create_t4_dx(df_ipd_s)
t4h_dx <- create_t4_dx(df_ipd_h)
t4_dx  <- tbl_merge(list(t4t_dx, t4n_dx, t4s_dx, t4h_dx),
                    tab_spanner = c("**Total**", "**Non-Sepsis**", "**Sepsis**", "**Septic shock**"))

t4t <- create_t4t(df_ipd)
t4n <- create_t4(df_ipd_n)
t4s <- create_t4(df_ipd_s)
t4h <- create_t4(df_ipd_h)
t4  <- tbl_merge(list(t4t, t4n, t4s, t4h),
                 tab_spanner = c("**Total**", "**Non-Sepsis**", "**Sepsis**", "**Septic shock**"))
